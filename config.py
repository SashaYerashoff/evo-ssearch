...
class Config:
    ...
    SIGLIP_CUDA_GRAPH_ENABLED = _get_bool_env(
    ...
    _L0_PROMPT_BUDGET_DEFAULTS = {
        ...
        'LUXRIOT_L0_HEARTBEAT_OUTPUT_TOKENS': (
            'EVOSSEARCH_LUXRIOT_L0_HEARTBEAT_OUTPUT_TOKENS', 320, 256, 32768
        ),
        'LUXRIOT_L0_EVENT_OUTPUT_TOKENS': (
            'EVOSSEARCH_LUXRIOT_L0_EVENT_OUTPUT_TOKENS', 384, 256, 32768
        ),
    ...
    try:
        LUXRIOT_BOOKMARK_COOLDOWN_SEC = float(os.getenv('EVOSSEARCH_LUXRIOT_BOOKMARK_COOLDOWN_SEC', '5.0'))
    except (TypeError, ValueError):
    ...
    try:
        LUXRIOT_ALERT_DEDUPE_WINDOW_SEC = float(
            os.getenv('EVOSSEARCH_LUXRIOT_ALERT_DEDUPE_WINDOW_SEC', '5.0')
        )
    except (TypeError, ValueError):
        LUXRIOT_ALERT_DEDUPE_WINDOW_SEC = 5.0
    ...
    try:
        PROBE_BOOKMARK_COOLDOWN_SEC = float(os.getenv('EVOSSEARCH_PROBE_BOOKMARK_COOLDOWN_SEC', '5.0'))
    except (TypeError, ValueError):
        PROBE_BOOKMARK_COOLDOWN_SEC = 5.0
    PROBE_BOOKMARK_COOLDOWN_SEC = max(0.0, PROBE_BOOKMARK_COOLDOWN_SEC)
    try:
        PROBE_BOOKMARK_DEDUPE_WINDOW_SEC = float(os.getenv('EVOSSEARCH_PROBE_BOOKMARK_DEDUPE_WINDOW_SEC', '5.0'))
    except (TypeError, ValueError):
        PROBE_BOOKMARK_DEDUPE_WINDOW_SEC = 5.0
    ...
    try:
        VLM_FAST_ALERT_COOLDOWN_SEC = float(
            os.getenv('EVOSSEARCH_VLM_FAST_ALERT_COOLDOWN_SEC', '5.0')
        )
    except (TypeError, ValueError):
        VLM_FAST_ALERT_COOLDOWN_SEC = 5.0
    ...
    try:
        VLM_FAST_ALERT_DEDUPE_WINDOW_SEC = float(
            os.getenv('EVOSSEARCH_VLM_FAST_ALERT_DEDUPE_WINDOW_SEC', '5.0')
        )
    except (TypeError, ValueError):
        VLM_FAST_ALERT_DEDUPE_WINDOW_SEC = 5.0
