...
Назначение: дать инженеру, PM и оператору простую картину физической раскладки
клиентского пилота: где Luxriot Evo, где EVA AI, где агентная модель, где VLM,
и какие основные связи должны работать.

Это не сетевой security-design и не inventory всех портов. Это рабочая схема
для установки, диагностики и разговора с клиентом.

SVG-картинка для открытия в браузере или вставки в документ:
`readiness/CLIENT_PHYSICAL_TOPOLOGY_0.8.3.svg`.

## 1. Узлы

| Узел | Роль | Известные параметры |
| --- | --- | --- |
| Luxriot Evo server | Источник каналов, archive, bookmarks | `[FIELD]`, observed pilot URL `http://192.168.3.27:8080` |
...
| LM Studio on EVA AI server | Агентная LLM | `qwen3.5-9b-mtp` |
