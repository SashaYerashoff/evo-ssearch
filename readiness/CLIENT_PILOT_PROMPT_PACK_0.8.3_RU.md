...
## 12. Сценарии внимания (гомеостаз канала)

`capture_attention` отмечает секунды, в которые измеренное движение заметно
превысило собственную норму конкретного канала. Это статистический сигнал для
выбора места дальнейшего просмотра, а не доказательство типа события. При
частоте 1 fps он также не гарантирует точное попадание в единственный
«идеальный» кадр быстрого действия.

### Найти резкие изменения на выбранном канале

```text
Что резкого происходило на канале [CHANNEL] за последний час? Сначала найди окна с capture_attention mode=burst, затем проверь самые сильные окна по описаниям и архивным кадрам. Отдели измеренный всплеск движения от визуального вывода о событии.
```

Expected: агент перечисляет burst-окна и `activity_x`, а смысл события называет
только после проверки доступных описаний или кадров.

### Показать самый сильный всплеск

```text
Найди самый сильный всплеск внимания на канале [CHANNEL] за [PERIOD]. Покажи время, activity_x, номера snapshots и открой лучший доступный архивный кадр; если есть burst_companion, используй его для более чёткого визуального подтверждения.
```

Expected: агент выбирает максимум по измеренному сигналу, не выдаёт размытый
apex за ошибку системы и предпочитает более чёткий companion для просмотра.

### Собрать короткую шкалу внимания

```text
Составь краткую временную шкалу burst-внимания для канала [CHANNEL] за [PERIOD]: объедини соседние всплески в окна, укажи пик activity_x и кратко напиши, что визуально подтверждено в каждом окне. Не заполняй пробелы догадками.
```

Expected: соседние секунды собраны в читаемые окна, а непроверенные пики явно
остаются кандидатами внимания.

### Проверить несколько каналов

```text
Проверь каналы [CHANNELS] за последние [PERIOD] и ранжируй до пяти самых сильных burst-окон. Обработай весь запрошенный период и все каналы, даже если инструменты требуют нескольких итераций; затем визуально проверь верхние кандидаты.
```

Expected: агент не ограничивается первой страницей или первой тройкой каналов и
разделяет межканальное ранжирование сигнала от выводов по кадрам.

### Отличить спокойный период от отсутствия данных

```text
Были ли всплески внимания на канале [CHANNEL] за [PERIOD]? Отдельно сообщи: найденные mode=burst, покрытие видео-описаниями и архивом, а также stale/frozen/down или иные пробелы. Не называй период спокойным, если данных недостаточно.
```

Expected: ответ различает «всплесков не отмечено» и «наблюдение отсутствовало
или было неполным».

### Проверить операторскую гипотезу

```text
Оператор предполагает, что около [TIME] на канале [CHANNEL] было [VISIBLE_EVENT_CANDIDATE]. Проверь capture_attention вокруг этого времени, затем описания и архивные кадры. Скажи, что поддерживает гипотезу, что ей противоречит и чего нельзя установить по этим данным.
```

Expected: burst используется как навигационная подсказка, а не как
семантическое или юридическое доказательство события.
