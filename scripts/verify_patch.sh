...
check_embedder_contract() {
  ...
  code="$(curl "${CURL_OPTS[@]}" -o "${body}" -w '%{http_code}' \
    "${BASE_URL}/ready" 2>/tmp/eva-patch-curl.err || true)"
  if [[ "${code}" != "200" ]]; then
...
assert embedder.get("required") is True, embedder
assert embedder.get("ok") is True, embedder
assert str(embedder.get("status") or "").casefold() == "loaded", embedder
PY
  then
    fail "required embedder did not reach the public loaded readiness contract"
    sed -n '1,8p' "${body}" >&2
    rm -f "${body}" /tmp/eva-patch-curl.err
    return 1
  fi
  rm -f "${body}" /tmp/eva-patch-curl.err
  ok "required embedder reached the public loaded readiness contract"
}

check_embedder_contract || RESULT=1

check_operator_semantic_contract() {
  local body code detail
  body="$(mktemp)"
  code="$(curl "${CURL_OPTS[@]}" -o "${body}" -w '%{http_code}' \
    "${BASE_URL}/ready" 2>/tmp/eva-patch-curl.err || true)"
  if [[ "${code}" != "200" ]]; then
    fail "could not inspect the public operator semantic contract (HTTP ${code:-000})"
    rm -f "${body}" /tmp/eva-patch-curl.err
    return 1
  fi
...
restore = checks.get("luxriot_restore", {}) or {}
assert attention.get("required") is True, attention
assert attention.get("ok") is True, attention
assert str(attention.get("status") or "").casefold() == "reachable", attention
if restore.get("required") is True:
    assert restore.get("ok") is True, restore
print(
    "attention storage is reachable; capture restore is "
    f"{restore.get('status') or 'not required'}"
)
PY
)"; then
