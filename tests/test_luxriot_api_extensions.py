...
def test_luxriot_client_exposes_archive_snapshot_and_timeline_methods():
    ...
    stream = client.open_live_stream(7)
    ...
    assert session.requests[3][2]["params"] == {"time": 3000, "streamType": "mainStream"}
    assert session.requests[4][1] == "http://luxriot.test/live/7/mainStream"
    assert session.requests[4][2]["stream"] is True
