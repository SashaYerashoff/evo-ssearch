...
| `EVOSSEARCH_SEMANTIC_PRESENCE_ENABLED` (`True`) | Enables shadow-only per-channel whole-frame text-affinity pulses over the pooled embedding already produced at archive cadence. The pulse reports deviation from that class prompt's channel baseline, never `present`/`absent`; it does not run another image encoder, create alerts, or regulate attention. Same-forward patch metadata is kept separately as top-patch-minus-patch-median localization contrast and cannot replace the whole-frame pulse |
...
| `EVOSSEARCH_PROBE_PATCH_ATTENTION_ENABLED` (`True`) | Enables explicit operator-requested SigLIP2 patch/text affinity for one exact buffered frame. Maps are ephemeral localization hints, are not object detections, and never enter whole-frame homeostasis, alerting, attention scheduling, or archive records |
...
| `EVOSSEARCH_LUXRIOT_L0_MAX_SELECTED_FRAMES` (`8`) | Total ordinary-L0 evidence-image budget, clamped to 4–8 and by the hard request limit. Attention ranks frames but stable observations at distinct times backfill any selection below four. The composer may reserve one slot for a stable companion of a CV apex; wider capture windows retain bounded-selection provenance. Normal attention compression is not itself a coverage gap; partial coverage is reserved for explicitly unrepresented salient evidence or source/decode loss. |
...
| `EVOSSEARCH_LIVE_CLIP_BATCH_SIZE` (`8`) / `_BATCH_WAIT_MS` (`8`) / `_BATCH_QUEUE_CAPACITY` (`128`) / `_BATCH_TIMEOUT_SEC` (`45`) | Cross-channel CLIP microbatch execution. The short gather window avoids adding operator latency when sparse channel phases do not align; latest-only per-channel dispatch bounds backlog while durable cadence counters expose omitted slots |
| `EVOSSEARCH_SIGLIP_CUDA_GRAPH_ENABLED` (`true`) | Replay the fixed batch-one 224px SigLIP vision graph on CUDA. This removes repeated kernel-launch overhead without changing preprocessing, weights, pooled embeddings, or same-forward patch outputs. Unsupported input shapes and capture failures stay on the eager path for the current encoder generation |
