...
- [x] Appearance editor возвращён в Settings как встроенная вкладка; отдельная
  кнопка в TopBar и отдельная modal-точка входа удалены. Preview, Reset, Cancel
  и Apply переиспользуют существующий редактор и сохранение предпочтений.
...
- [x] Общее количество archive matches показывается над результатами по центру;
  отдельно отображается количество уже загруженных записей.
