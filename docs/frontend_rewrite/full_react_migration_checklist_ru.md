...
- [x] Full-screen агента использует непрозрачную поверхность и отдельный solid
  backdrop над workspace; нижний status console в этом режиме исключён из
  layout. Слева и справа сохранён системный gutter шириной edge-trigger + 12 px,
  поэтому «уши» главного меню и AGENT не перекрывают содержимое dock.
