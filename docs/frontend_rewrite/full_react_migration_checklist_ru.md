...
- [x] Docked EVA Agent использует фиксированные width-пресеты: Full HD держит
  ...
  переходят на следующую строку вместо сжатия. В режиме `Big 150%` пресеты и
  drag-нормализация используют внутренний layout viewport (`window / 1.5`), а
...
- [x] На Archive, Stream Summaries и Probes шапка вкладок и фильтров закреплена
  ...
  на общем фоне страницы, а заливка, рамка и тень принадлежат только
  `atp-tabpanel-content`. Сам `.tool-tabs.with-leading` также не имеет фоновой
  полосы или нижней границы; собственная заливка сохранена только у кликабельных
  `.atp-tab`.
- [x] Глобальный trigger меню заменён компактной edge-рейкой
  `.menu-rail-trigger`: она показывает иконки доступных разделов, Settings и
  Logout, подсвечивает активный раздел и целиком открывает полноразмерное меню.
  На Home рейка рендерится через `LeftRail`, а tabbed-разделы передают тот же
  компонент в leading-колонку `ToolTabs`. Leading-колонка имеет ширину `28px`,
  а рейка растягивается ровно по высоте `.atp-tabpanel-content`, поэтому её
