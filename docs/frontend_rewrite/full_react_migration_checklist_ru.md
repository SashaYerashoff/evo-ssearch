...
- [x] На Archive, Stream Summaries и Probes шапка вкладок и фильтров закреплена
  ...
  В уникальной панели Image действие `Choose image` закреплено у крайнего левого
  края рабочей области, а не в общей правой зоне основных toolbar-действий.
  В панели Text Query нет отдельной внешней кнопки `Search`: компактная лупа
  встроена в правый край поля, а запрос запускается кликом по ней или клавишей
  `Enter`. Во время запроса лупа становится spinner, повторный submit блокируется,
  а после успешного применения запроса она превращается в красный крестик.
  Изменение применённого текста сразу возвращает лупу; крестик или очистка поля
  клавиатурой автоматически снимают текстовый фильтр и загружают обычный архив.
  Пока результаты ограничены текстовым запросом, рядом с общими счётчиками
  отображается метка `FILTERED` без chip-фона и рамки, на общей базовой линии;
  она исчезает вместе со сбросом фильтра.
  Текущая сетка карточек приглушается и пульсирует единым поисковым слоем без
  изменения layout; reduced/no-motion оставляет статичное приглушение. Строка
  вида `N matches · “query”` после поиска не выводится: число уже есть в общем
  счётчике результатов, а текст запроса остаётся только в поле ввода.
  Под контролами активного `.atp-tabpanel-content` постоянно течёт едва заметный
  северный «Биврёст»: широкая холодная аура и две переплетающиеся спектральные
  жилы из бирюзы, ледяного синего, фиолетового и слабого золотого света. Бесшовный
  поток покрывает всю ширину без пустых пауз. Слева видимая область физически
  обрезана скруглением, а фиксированный fade длиной 12–14 px скрывает острые углы
  без заметной потери световой площади независимо от ширины окна;
  справа поток растворяется маской, не перехватывает ввод и отключается общим режимом
  reduced/no motion. Сам
...
- [x] Закреплённые action-группы Archive, Stream и Probes не маскируют световую
  подложку `.atp-tabpanel-content`: «река света» непрерывно проходит под кнопками
  и промежутком между `Actions` и главным действием, сохраняя их фиксированную
  позицию справа.
...
- [x] Внутренний интерфейс Agent приведён к стеклянной теме: обычные действия
  приглушены, активные состояния сохраняют accent, `Research trace` отображается
  световой направляющей вместо отдельной карточки, action cards и textarea имеют
  локальное стекло, а нижний composer растворяется в transcript без жёсткой
  горизонтальной полосы. Resize/snap/fullscreen-геометрия при этом не изменена.
- [x] Resize-handle агента совпадает с реальной областью взаимодействия: по всей
  высоте dock проходит тонкая спокойная линия, которая раскрывается в узкую
  стеклянную полосу только при hover/drag. Отдельная центральная капсула и
  декоративные точки удалены; ширина фактической области захвата не изменена.
...
- [x] Глобальный trigger меню заменён компактной edge-рейкой
  ...
  компактный знак из трёх одинаковых рядов «точка + линия» и целиком открывает
  полноразмерное меню.
  Закрытая рейка использует тот же растворяющийся стеклянный слой, что drawer:
  плотная прямоугольная заливка и общий контур убраны, справа остаётся тонкий
  световой шов, а три одинаковых ряда знака мягко усиливаются только на hover.
...
- [x] Полноразмерное меню продолжает занимать высоту рабочей зоны, чтобы `Log out`
  оставался закреплён снизу, но больше не выглядит сплошной таблицей: поверхность
  растворяется к правому краю над neural background, общий контур и горизонтальные
  рамки убраны. Активный пункт отмечает локальная световая река с тонкой левой
  accent-линией; hover использует более слабую версию того же эффекта, а подложка
  остаётся только у иконки. Заголовок `Navigation` и separator также заканчиваются
  мягким градиентом вместо жёстких линий.
- [x] `Stream summaries` оформлен как свободная событийная лента на общем neural
  background вместо большой таблицы внутри общей карточки. Внешняя feed-панель,
  полные рамки summary и жёсткие разделители убраны; каждая запись получает
  растворяющуюся вправо поверхность, мягкую левую accent-линию и локальный слой
  под narrative. Telemetry остаётся компактной строкой, нейтральные chips облегчены,
  actions приглушены до hover/focus, а coverage gaps сохраняют warning-тон.
  Дублирующий heading `Stream summaries` с повтором выбранного канала, depth,
  количества записей и live/fixed-состояния удалён: лента начинается сразу с данных.
- [x] `Incident review` приведён к той же облегчённой схеме: Channel, Period и Refresh
  перенесены в верхнюю рабочую панель вкладки без подписей над контролами, а
  дублирующий заголовок и описание внутри раздела удалены. Внешняя монолитная карточка
  с заливкой и рамкой убрана; очереди, empty/error state и incident-карточки лежат на
  общем neural background. Состояния очередей и карточек отмечаются локальным accent,
  а не дополнительными табличными контейнерами.
- [x] Рабочая область `Stream settings` разгружена от двух тяжёлых карточек: preview
  отображается как самостоятельное визуальное окно, а `Runtime and attention` — как
  диагностическая лента на общем фоне. Основные состояния собраны в компактную сетку
  с мягкими разделителями, длинные runtime-параметры оформлены свободными строками,
  а `Last error` получает только локальный danger-маркер без рамки вокруг всего блока.
...
- [x] Архивные `DetectionCard` адаптированы под стеклянную тему без потери читаемости
  кадров: прозрачный glass-слой применяется к корпусу и metadata, источник получает
  локальную blur-плашку, а между preview и подписью добавлен мягкий световой переход.
  Контур и glow остаются приглушёнными и усиливаются только на hover/focus; стиль
  изолирован классом `.archive-card` и не меняет остальные универсальные `.card`.
...
- [x] Контролы `Stream settings` приведены к общей toolbar-геометрии: компактные
  поля имеют высоту `30px`, основные action-кнопки — `38px`, а ширины
  Channel/Batch/Every/Model ограничены `240/74/76/200px`; поля больше не
  растягиваются по всей ширине экрана.
  Dropdown-контролы Stream Review/Settings используют тот же компактный pill/chip
  стиль, что и Archive; числовые и датовые поля имеют тот же скруглённый силуэт.
  Видимые групповые и полевые подписи над toolbar-контролами удалены; их названия
  сохранены как `title`/`aria-label`, а элементы собраны в одну компактную строку.
...
- [x] Probe board адаптирован под облегчённую glass-тему: полноширинные group/channel-таблицы заменены лёгкими направляющими, а карточки получили прозрачный материал и локальную статусную подсветку.
...
- [x] Общая оболочка Settings приведена к glass-схеме интерфейса: монолитная заливка
  заменена полупрозрачным полотном с мягким световым потоком, навигация получила
  компактную стеклянную рейку, секции и footer растворяются в фоне, а контуры
  сохранены только у интерактивных полей и действий.
