...
- [x] На Archive, Stream Summaries и Probes шапка вкладок и фильтров закреплена
  ...
  В панели Text Query отдельная кнопка `Search` отсутствует: запрос запускается
  клавишей `Enter` из текстового поля, повторный submit во время поиска блокируется.
