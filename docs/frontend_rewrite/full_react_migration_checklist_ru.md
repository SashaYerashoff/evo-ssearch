...
- [x] Appearance editor возвращён в Settings как встроенная вкладка; отдельная
  кнопка в TopBar и отдельная modal-точка входа удалены. Reset, Cancel и Apply
...
- [x] Секции Appearance расположены одной вертикальной колонкой; демонстрационный
  ...
  удалены. Сохраняемый режим `Normal / Big 150%` вынесен в отдельную первую
...
- [x] Docked EVA Agent использует фиксированные width-пресеты: Full HD держит
  ...
  переходят на следующую строку вместо сжатия. После максимального пресета
...
- [x] На Archive, Stream Summaries и Probes крупная кнопка MENU встроена в общую
  шапку вкладок и фильтров; шапка закреплена сверху и не прокручивается вместе
  с рабочим содержимым. Кнопка является отдельным trigger старого глобального
  navigation drawer и скрывается при его открытии, не меняя сам drawer. Такой
  же trigger доступен на Home; подпись везде единая — `MENU`. Trigger пристыкован
  к левому краю экрана и является отдельным соседом панели. Вкладки разделов
  (`Stream review` / `Stream settings` и аналоги) находятся внутри `atp-tabpanel`
  над фильтрами, а не рядом с `menu-header-button`. Внешний `atp-tabpanel` и
  `atp-tabrow` прозрачны: вкладки лежат на общем фоне страницы, а заливка, рамка
  и тень панели принадлежат только `atp-tabpanel-content` с фильтрами. Сам
  `.tool-tabs.with-leading` также не имеет фоновой полосы или нижней границы;
  собственная заливка сохранена только у кликабельных `.atp-tab`. Кнопка `MENU`
  во всех рабочих разделах использует тот же компактный размер и визуальный стиль,
  что и trigger на Home. Для Home и всех tabbed-разделов используются единые
  высота `56px` и вертикальный offset; положение не зависит от высоты содержимого вкладки.
  В tabbed-разделах кнопка отступает от края рабочей области и центрируется по
  высоте `.atp-tabpanel-content`, а не по строке вкладок. Рамка, скругления и
  hover-подсветка симметричны с обеих сторон. Home trigger использует те же
  отступы `16px/56px`, поэтому не прыгает при переходе между разделами.
